...
import { Search, ChevronLeft, ChevronRight, Download, Users, DollarSign, TrendingDown, Stethoscope } from "lucide-react";
...
export default function FinanceDebtorPatientsPage() {
    const navigate = useNavigate();
    const [patients, setPatients] = useState([]);
    const [pagination, setPagination] = useState({ page: 1, pages: 1, total: 0 });
    const [stats, setStats] = useState({ totalDebt: 0, totalPaid: 0, totalPerformed: 0, debtorCount: 0 });
    ...
    const fetchData = useCallback(async (p) => {
        try {
            ...
            const res = await fetch(`/api/billing/debtor-patients?${params}`); const data = await res.json();
            setPatients(data.patients || []); setPagination(data.pagination || { page: 1, pages: 1, total: 0 }); setStats(data.stats || { totalDebt: 0, totalPaid: 0, debtorCount: 0 });
    ...
    const handleExport = async () => {
        ...
        if (appliedSearch) params.set("search", appliedSearch);
        const res = await fetch(`/api/billing/debtor-patients?${params}`); const data = await res.json();
        const rows = (data.patients || []).map(p => ({ Hasta: `${p.firstName} ${p.lastName}`, Telefon: p.phone || "-", Hekim: p.doctor || "-", "Planlanan": (p.totalPlanned / 100).toFixed(2), "Yapılan": (p.performedCost / 100).toFixed(2), "Ödenen": (p.totalPaid / 100).toFixed(2), "Borç": (p.balance / 100).toFixed(2), "Son Randevu": p.lastAppointmentDate ? format(new Date(p.lastAppointmentDate), "dd.MM.yyyy", { locale: tr }) : "-" }));
        const ws = XLSX.utils.json_to_sheet(rows); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Borçlu Hasta"); XLSX.writeFile(wb, `borclu_hasta_${format(new Date(), "yyyy-MM-dd")}.xlsx`);
    };

    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between"><div><h2 className="text-2xl font-bold tracking-tight">Borçlu Hasta Listesi</h2><p className="text-xs text-muted-foreground mt-0.5">Ödenmemiş bakiyesi olan hastalar</p></div>
            ...
            <div className="grid gap-4 md:grid-cols-4">
                <div className="glass-effect rounded-xl p-4 border border-border/50"><div className="flex items-center gap-2 text-muted-foreground mb-1"><TrendingDown className="h-4 w-4 text-red-500" /><span className="text-sm font-medium">Toplam Borç</span></div><div className="text-2xl font-bold text-red-600">{formatCurrency(stats.totalDebt)}</div></div>
                <div className="glass-effect rounded-xl p-4 border border-border/50"><div className="flex items-center gap-2 text-muted-foreground mb-1"><Stethoscope className="h-4 w-4 text-blue-500" /><span className="text-sm font-medium">Yapılan Tedavi</span></div><div className="text-2xl font-bold text-blue-600">{formatCurrency(stats.totalPerformed)}</div></div>
                ...
                <div className="glass-effect rounded-xl p-4 border border-border/50"><div className="flex items-center gap-2 text-muted-foreground mb-1"><Users className="h-4 w-4 text-orange-500" /><span className="text-sm font-medium">Borçlu Hasta</span></div><div className="text-2xl font-bold">{stats.debtorCount}</div></div>
            </div>

            <div className="glass-effect rounded-xl border border-border/50 px-4 py-3">
                <div className="relative max-w-sm"><Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground" /><Input className="pl-8 h-8 text-xs" placeholder="Hasta adı veya telefon ara..." value={search} onChange={e => onSearch(e.target.value)} /></div>
            </div>

            <div className="glass-effect rounded-xl overflow-hidden border border-border/50">
                {loading ? <div className="flex items-center justify-center h-40 text-muted-foreground text-sm">Yükleniyor...</div> : patients.length === 0 ? <div className="flex flex-col items-center justify-center py-16 text-muted-foreground gap-2"><p className="text-sm">Borçlu hasta bulunamadı</p></div> : <>
                    <div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-muted/20 text-muted-foreground"><tr>
                        ...
                        <th className="py-2.5 px-4 text-left text-[11px] uppercase tracking-wide font-medium">Telefon</th>
                        ...
                        <th className="py-2.5 px-4 text-left text-[11px] uppercase tracking-wide font-medium">Son Randevu</th>
                            ...
                            {patients.map(p => <tr key={p.id} className="hover:bg-muted/5 cursor-pointer" onClick={() => navigate(`/patients/${p.id}?tab=payments`)}>
                                ...
                                <td className="py-2.5 px-4 text-muted-foreground">{p.phone || "—"}</td>
                                ...
                                <td className="py-2.5 px-4 text-right text-blue-600">{formatCurrency(p.performedCost)}</td>
                                <td className="py-2.5 px-4 text-right text-emerald-600">{formatCurrency(p.totalPaid)}</td>
                                <td className="py-2.5 px-4 text-right font-bold text-red-600">{formatCurrency(p.balance)}</td>
                    ...
                    <div className="px-4 py-3 border-t border-border/40 bg-muted/5 flex items-center justify-between">
                        <div className="text-xs text-muted-foreground">Toplam <span className="font-semibold text-foreground">{pagination.total}</span> borçlu hasta</div>
